  ...
  try {
    ...
    const prompt = `
      ...
      จงดึงข้อมูลออกมาให้อยู่ในรูปแบบ JSON เท่านั้น โดยมีโครงสร้างดังนี้:
      {
        ...
        "date": "วันที่บนบิล (บิลในประเทศไทยมักเขียนในรูปแบบ วัน/เดือน/ปี หรือ DD/MM/YYYY ให้ถือว่าตัวเลขชุดแรกคือ 'วัน' และชุดที่สองคือ 'เดือน' เสมอ จากนั้นแปลงผลลัพธ์ให้อยู่ในรูปแบบ YYYY-MM-DD เพื่อส่งกลับมา เช่น 2026-03-04 หากปีเป็นพ.ศ.ให้แปลงเป็นค.ศ. ถ้าหาไม่เจอให้เป็นค่าว่าง)",
        "items": [
          {
      ...
      หากราคาในบิลไม่มีการระบุจำนวนชิ้น (quantity) ให้บังคับใส่ quantity เป็น 1
      ตรวจสอบให้แน่ใจว่าผลลัพธ์เป็น JSON ล้วนๆ ห้ามมี markdown (เช่น \`\`\`json) หรือคำบรรยายใดๆ ปนมา
    `;

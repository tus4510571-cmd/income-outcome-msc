...
export default function NewChatDirectPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const customFileName = `${dateStr}${dailySeq}-IN-ในประเทศ-${safeCustomerName}`;
      ...
      router.push("/income/chat-direct");
    } catch (err) {
    ...
    <main className="min-h-screen p-4 md:p-8 bg-slate-50/50">
      <div className="max-w-3xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-slate-500 mt-1">ลูกค้าจากประเทศไทย คุยตรง</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="card space-y-6">
          ...
          <div className="space-y-4 pt-4 border-t border-slate-100 dark:border-slate-700">
             ...
             <p className="text-sm text-slate-500">คุณสามารถอัปโหลดสลิปโอนเงิน หรือหลักฐานอื่นๆ ที่เกี่ยวข้องได้ที่นี่</p>
            <PreImageUpload
              files={uploadFiles}
          ...
          <div className="pt-4 flex gap-3">
             <button
               type="button"
               onClick={() => router.push("/income/chat-direct")} 

...
- **Immediate Incomplete Transaction on Employee Receipt Creation (2026-09-03):** Saving an employee receipt voucher in `outcome/employee-labor/receipt/new` now immediately creates a transaction in Supabase with category `employee_labor`. Because files (slip, ID card copy, signed voucher, summary) have not been uploaded yet, it is marked as `incomplete` and immediately appears in Transaction Summary (`/outcome/summary`) under "รายการขาดเอกสาร" (missing documents) and on `/outcome/employee-labor` so users can return to upload remaining files anytime.
